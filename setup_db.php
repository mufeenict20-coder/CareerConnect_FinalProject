...
try {
    ...
    // 3. Insert Default Admin Account (Password: admin123)
    $adminPassword = password_hash('admin123', PASSWORD_BCRYPT);
    $adminStmt = $db->prepare("
        INSERT OR IGNORE INTO users (user_id, name, email, password, role) 
        VALUES (1, 'System Admin', 'admin@careerconnect.ac.lk', :password, 'Admin')
    ");
    $adminStmt->execute([':password' => $adminPassword]);

    echo "<div style='font-family: sans-serif; padding: 20px; text-align: center;'>";
    echo "<h2 style='color: #16a34a;'>✅ Success! SQLite database created successfully.</h2>";
    echo "<p>Your <code>careerconnect.sqlite</code> file is initialized and ready for use.</p>";
    echo "<a href='login.php' style='display: inline-block; padding: 10px 20px; background: #2563eb; color: #fff; text-decoration: none; border-radius: 5px;'>Go to Login Page</a>";
    echo "</div>";

} catch (Exception $e) {
    echo "<div style='font-family: sans-serif; padding: 20px; text-align: center;'>";
    echo "<h2 style='color: #dc2626;'>❌ Database Setup Error</h2>";
